...
class User_Bg(ModelForm):
    ...
    birthday = forms.DateField(
        label='Birthdate (m/d/y)', input_formats=['%m/%d/%y'])
...
class Client_List(ModelForm):
    clients = forms.ModelMultipleChoiceField(queryset=Account.objects.filter(
        is_client=True, has_caseworker=False), widget=forms.CheckboxSelectMultiple)
...
class Remove_Clients(ModelForm):
    clients = forms.ModelMultipleChoiceField(
        queryset=Account.objects.none(), widget=forms.CheckboxSelectMultiple)
...
class Add_Individual(ModelForm):
    ...
    zipcode = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Zip Code'}), required=True)
    description = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Description'}), required=True)
    services_provided = forms.ModelMultipleChoiceField(
        queryset=ServicesProvided.objects.all(), widget=forms.CheckboxSelectMultiple)
    skills_expertise = forms.ModelMultipleChoiceField(
        queryset=SkillsExpertise.objects.all(), widget=forms.CheckboxSelectMultiple)
...
class Add_Organization(ModelForm):
    org_name = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Organization Name'}), required=True)
    phone_number = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Phone Number'}), required=True)
    email = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Email'}), required=True)
    website = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Website'}), required=True)
    address = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Address'}), required=True)
    city = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'City'}), required=True)
